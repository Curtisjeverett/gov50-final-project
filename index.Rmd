...
pennsylvania_counties <- read_csv("PA Counties.csv") |>
  ...
  mutate(Higher_Income = if_else(`Higher Income` == 1, "Higher", "Lower"), 
         High_Minority = if_else(`Higher proportion minority` == 1, "High prop", "Low prop"))
...
```{r}
pennsylvania_counties |>
  ggplot(aes(x = `2020 Personal Income Per Capita (in dollars)`,
             y = `2018 Midterm turnout (%)`)) +
  geom_point()

pennsylvania_counties |>
  ggplot(aes(x = prop_Minorities,
             y = `2018 Midterm turnout (%)`)) +
  geom_point()
...
income_ate <- pennsylvania_counties |> 
  specify(`2018 Midterm turnout (%)` ~ Higher_Income) |>
  calculate(stat = "diff in means", order = c("Higher", "Lower"))
income_ate

minority_ate <- pennsylvania_counties |>
  specify(`2018 Midterm turnout (%)` ~ High_Minority) |>
...
```{r}
turnout_changes <- lm(`2018 Midterm turnout (%)` ~ Higher_Income + High_Minority, data = pennsylvania_counties)
turnout_changes                 
